...
fillRestaurantHTML = (restaurant = self.restaurant) => {
  ...
  //Update indexedDB reviews
  DBHelper.populateReviewInfoDatabase(self.restaurant.id, self.restaurant);
}
...
fillReviewsHTML = (error, reviews) => {
  ...
  reviewLink.setAttribute("id", "new-review-link");
  reviewLink.innerHTML = 'Add Review';
  reviewLink.onclick = function() {
    window.location = `./restaurant_reviews.html?id=${self.restaurant.id}`; 
  }
...
createReviewHTML = (review) => {
  ...
  var timeCreated = review.createdAt;
  date.innerHTML = new Date(timeCreated).toLocaleString('en-US', options);
  li.appendChild(date);
